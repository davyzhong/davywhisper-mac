...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "DavyWhisper", category: "PluginRegistry")
...
final class PluginRegistryService: ObservableObject {
    ...
    private let registryURL = URL(string: "https://davywhisper.github.io/davywhisper-mac/plugins.json")!
    ...
    func fetchRegistry() async {
        ...
        do {
            ...
            // Fall back to bundled plugins.json when remote fetch fails
            if let bundledURL = Bundle.main.url(forResource: "plugins", withExtension: "json"),
               let bundledData = try? Data(contentsOf: bundledURL),
               let response = try? JSONDecoder().decode(PluginRegistryResponse.self, from: bundledData) {
                let appVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "0.0"
                registry = response.plugins.filter {
                    Self.compareVersions($0.minHostVersion, appVersion) != .orderedDescending
                        && $0.isCompatibleWithCurrentOS
                ...
                lastFetchDate = Date()
                fetchState = .loaded
                logger.info("Loaded \(self.registry.count) plugin(s) from bundled registry (remote unavailable)")
            } else {
