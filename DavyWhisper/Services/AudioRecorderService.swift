...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "davywhisper-mac", category: "AudioRecorderService")
...
final class AudioRecorderService: ObservableObject, @unchecked Sendable {
    ...
    private func startSystemAudioRecording(outputURL: URL) async throws {
        ...
        try stream.addStreamOutput(output, type: .audio, sampleHandlerQueue: DispatchQueue(label: "com.davywhisper.system-audio", qos: .userInteractive))
