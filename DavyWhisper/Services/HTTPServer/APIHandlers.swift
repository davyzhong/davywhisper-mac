...
private let apiLogger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "davywhisper-mac", category: "APIHandlers")
...
final class APIHandlers: @unchecked Sendable {
    ...
    private func handleTranscribe(_ request: HTTPRequest) async -> HTTPResponse {
        ...
        guard hasEngine else {
            return .error(status: 503, message: "No engine selected. Select an engine in DavyWhisper first.")
        }
        ...
        do {
            ...
            return .error(status: 500, message: "Transcription failed. Check app logs for details.")
        }
    }
