...
enum CLIError: Error {
    ...
    var message: String {
        ...
        case .connectionFailed(let port):
            return """
                Error: Cannot connect to DavyWhisper on port \(port).

                Make sure DavyWhisper is running and the API server is enabled:
                  1. Open DavyWhisper
        ...
        case .serverError(let code, let message):
            if code == 503 {
                return "Error: No model loaded in DavyWhisper. Load a model first."
            }
